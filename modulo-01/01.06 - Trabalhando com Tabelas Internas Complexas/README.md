...
2. Utilizar **Expressões de Tabela** (lt\_tab\[ ... \]) para ler registros, realizar encadeamentos diretos e gerenciar exceções.  
...
Diferente do READ TABLE que apenas define sy-subrc \= 4 quando falha, a expressão de tabela lança uma exceção CX\_SY\_ITAB\_LINE\_NOT\_FOUND.
...
#### **Funções Predicativas: line\_exists e line\_index**
...
* DATA(lv\_idx) \= line\_index( lt\_tab\[ key \= val \] ).: Retorna o índice (sy-tabix) ou 0 se não achar.
...
CLASS zcl\_itab\_demo IMPLEMENTATION.
  ...
  METHOD if\_oo\_adt\_classrun\~main.
    ...
    " Usando a chave 'sk\_cat', o acesso vira Binário O(log n).  
    ...
    TRY.  
        ...
        out-\>write( |Novo preço do Monitor: { lt\_products\[ id \= 'P003' \]-price }| ).  
    CATCH cx\_sy\_itab\_line\_not\_found.  
...
* **Table Expression (itab\[...\]):** Sintaxe funcional para leitura de registros. Lança exceção cx\_sy\_itab\_line\_not\_found em caso de falha, ao contrário do sy-subrc.  
...
| **Verificar** | READ TABLE ... TRANSPORTING NO FIELDS. | IF line\_exists( tab\[...\] ). | Código mais legível. |
| **Obter Índice** | READ TABLE ... depois verificar sy-tabix | idx \= line\_index( tab\[...\] ). | Acesso direto ao metadado. |
...
Q2: O que acontece se eu tentar acessar lt\_tabela\[ id \= '999' \] e o registro não existir?  
R: O sistema lançará uma exceção da classe cx\_sy\_itab\_line\_not\_found. Se não for tratada com TRY...CATCH, causará um Short Dump (erro em tempo de execução).  
...
R: Definindo uma Chave Secundária (WITH NON-UNIQUE SORTED KEY sk\_data COMPONENTS data) na definição da tabela e utilizando USING KEY sk\_data nas leituras e loops.  
